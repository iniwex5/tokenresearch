export default {
  ...
  "planPro": "Pro（292）",
  "planTeam": "Team（332）",
  "planHint": "手动选择套餐。票据需符合所选长度，并通过实际返回模型和全局 STATE 代理验证。更改套餐后需重新获取。",
  ...
  "globalPoolHint": "这里只控制本账号是否使用票据。获取和验证统一使用全局 STATE 代理池；日常请求有账号代理时仍走账号代理。",
  "proxy": "采集代理",
  "proxyPlaceholder": "粘贴完整代理 URL（包含用户名和密码）",
  "proxyConfigured": "已配置：{address}。留空保存会保留现有连接。",
  "proxyHint": "STATE 获取和验证只使用全局代理池。日常请求有账号代理时仍使用账号代理；票据验证通过后才使用，约一小时有效并提前自动刷新。",
